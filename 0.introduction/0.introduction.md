...
- `-d` --> **detached**. O container é executado e o terminal não é interrompido.
...
docker run --name computador1 -d -p 65:8080 69bd221063cf; # startamos o computador virtual()
```
...
Perceba que a porta `65` do seu computador local consegue acessar a porta `8080` do computador virtual(container).
